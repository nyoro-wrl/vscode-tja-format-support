 ...
 * ヘッダーの記載位置
 */
type HeaderLocation = "Main" | "Course";

/**
 * ヘッダー
 */
interface IHeader extends IStatement {
  readonly location: HeaderLocation;
}
...
class HeaderCollection extends StatementCollection<IHeader> {
  constructor(readonly items: IHeaderRecord) {
    super(items);
  }
}

export const headerCollection = new HeaderCollection({
  // 太鼓さん次郎
  title: {
    regexp: /^TITLE$/,
