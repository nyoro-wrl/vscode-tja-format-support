## 1.5.7 (2026/09/04)

- 追加
  - イモ連打の5,000点ボーダー指定に対応
    - 開始の`9`から終了の`8`までに置かれた2個目の`9`をボーダーとして扱います
    - 3個目以降の`9`は不正なパターンとして警告します

## 1.5.6 (2025/08/12)

- 追加
  - 多言語対応の推進

## 1.5.5 (2025/08/11)

- 追加
  - 多言語対応
    - English
    - 中文

## 1.5.4 (2025/08/06)

- 追加
  - 特殊音符への対応
    - `D`: 時限爆弾
  - 譜面情報に連打、風船音符、時限爆弾の情報を追加

## 1.5.3 (2025/07/27)

- 修正
  - ゴーゴータイムと譜面分岐の色付けが機能していなかった問題の修正

## 1.5.2 (2025/07/25)

- 仕様変更
  - 譜面内のヘッダーを色付け対象を追加
  - `EXAM`ヘッダーを譜面内に書けるように変更

## 1.5.1 (2025/07/25)

- 追加
  - ヘッダー/命令を追加
  - ヘッダー/命令に署名ヘルプ機能を追加
  - ヘッダー/命令にパラメーター補完を追加
- 仕様変更
  - ヘッダー/命令の補完条件を緩和
- 修正
  - 細かい修正

## 1.5.0 (2025/07/22)

- 追加
  - 小節番号を行末尾に表示する機能を追加（拡張機能設定から変更可能）
  - 風船音符`70008`から打数を直接書き換えられる機能の追加（カーソルを合わせてF2）
  - 不要な命令の警告を追加
  - 風船音符の定義が余っている場合の警告を追加
  - ファイルパスを指定するヘッダー（`WAVE:`など）でのファイルパス補完機能を追加
  - 各プレイヤー固有の命令（TJAPlayerやTaikoManyGimmicksなど）補完の表示/非表示設定の追加（拡張機能設定から変更可能）
    - TaikoManyGimmicksの命令についてはデフォルト非表示、`.tmg`拡張子においては強制有効です
  - 譜面の範囲編集機能の追加
    - ドン/カッを反転: ドンとカッを入れ替えます。
    - ドン/カッをランダム: ドンとカッを50%の確率で入れ替えます。
    - スクロール速度の遷移: スクロール速度を開始値から終了値に徐々に遷移させます。
- 仕様変更
  - 譜面の範囲編集機能の削除
    - ドン/カッの反転: n%の確率でドンとカッを入れ替えます。
- 修正
  - 譜面分岐時の小節カウントのバグを修正
  - 「譜面の切り詰め」機能の改善
  - 細かい修正

## 1.4.1 (2023/11/10)

- 修正
  - 1.4.0で発生していた拡張機能の立ち上げに失敗する重大なバグを修正

## 1.4.0 (2023/11/10)

- 追加
  - 譜面の範囲編集機能を追加（範囲選択→右クリック）
    - 譜面の拡大: 譜面の長さをn倍にします
    - 譜面の切り詰め: 余分な休符を削除します
    - スクロール速度の一定化: 指定したBPMを基準にスクロール速度を一定化します
    - 大音符に置換: 大音符に置き換えます
    - 小音符に置換: 小音符に置き換えます
    - 休符に置換: 休符に置き換えます
    - ドン/カッの反転: n%の確率でドンとカッを入れ替えます。
    - 命令の一括削除: すべての命令（または1つの種類の命令）を削除します
- 仕様変更
  - 主要なバージョンアップ時に更新通知を行うように変更
- 修正
  - 細かい修正

## 1.3.5 (2023/11/04)

- 修正
  - tmg形式への対応をちょっとだけ改善
- 仕様変更
  - 譜面の0埋め入力補完をより使いやすくしました
    - 変更前
      - 直前の小節（複数行であればその全体）と同じ長さで0埋め
    - 変更後
      - 直前の行と同じ長さで0埋め

## 1.3.4 (2023/08/26)

- 追加
  - .tmg形式に暫定対応
    - 大抵は動くと思いますがバグがあるかもしれません

## 1.3.3 (2023/08/26)

- 追加
  - 特殊音符への対応
    - `A`: 手繋ぎ音符（ドン）
    - `B`: 手繋ぎ音符（カッ）
    - `C`: 爆弾音符
    - `F`: アドリブ
    - `G`: カドン
- 修正
  - 連打音符（大）の色が想定と異なっていたのを修正
- 仕様変更
  - 特殊音符の対応に合わせてA~Zの文字を音符扱いにしました

## 1.3.2 (2023/04/08)

- 追加
  - 構文解析をカットして動作を軽量化するモードを追加（ステータスバーまたは設定から）
    - 主にギミック譜面などで使うかも？
- 修正
  - ダミーの風船が通常の風船と同じ扱いになっていたのを修正
  - ファイルを閉じても診断情報が消えない問題を修正
  - 構文解析が#RESETCOMMANDに対応しました
  - 細かい修正
- 仕様変更
  - 設定を日本語化

## 1.3.1 (2023/03/31)

- 追加
  - 譜面情報を追加（左部サイドバー → エクスプローラー → 譜面情報）
    - 最大コンボ数などが確認できます
  - ステータスバーにカーソル位置のコンボ数表示を追加
  - パンくずリストに譜面分岐の区分を追加
  - パンくずリストに段位道場の課題曲を追加
  - 譜面の0埋め入力補完を試験的に追加（直前の小節の長さに合わせます）
  - 折りたたみ機能を譜面分岐でも使えるように拡張
  - 折りたたみ機能を段位道場の課題曲でも使えるように拡張
  - カラーピッカー機能を追加
    - DANTICKCOLOR:ヘッダーと#COLOR命令で使えます
  - 拡張機能の設定項目を追加
    - ゴーゴータイム色付けのオン･オフ
    - 譜面分岐色付けのオン･オフ
- 修正
  - 入力補完の修正
  - 細かい修正
- 仕様変更
  - 色付け処理を少し厳密化
- 削除
  - ステータスバーから譜面分岐の区分表示を削除
    - 代わりにパンくずリストを参照してください

## 1.3.0 (2023/03/15)

- 追加
  - 譜面分岐の構文解析に対応
  - ゴーゴータイムに色付けを追加
  - 譜面分岐に色付けを追加
  - 風船関連の機能が譜面分岐に対応
  - カーソルを合わせた位置の譜面分岐の区分がわかるようになりました（下部ステータスバーに表記）
  - 譜面分岐の区分を移動できるようになりました（下部ステータスバー譜面分岐区分クリック）
- 修正
  - 譜面分岐の場合に誤った小節数が表示されていたのを修正
  - 細かい修正

## 1.2.2 (2023/03/12)

- 追加
  - 風船音符の打数が指定されていない場合に警告を表示するようにしました
- 修正
  - 同一難易度で一人プレイ用と二人プレイ用が分けられている場合の構文解析を実装しました
  - 細かい修正

## 1.2.1 (2023/03/11)

- 追加
  - 風船音符にマウスを合わせると風船の打数を表示する機能を実装しました
  - 風船打数にカーソルを合わせてF12（または右クリック→定義へ移動）を押すと風船音符に直接移動できる機能を実装しました
  - 風船音符にカーソルを合わせてF12（または右クリック→定義へ移動）を押すと打数に直接移動できる機能を実装しました
- 修正
  - 細かい修正
- 既知の不具合
  - 風船関連の機能が譜面分岐に対応していません

## 1.2.0 (2023/03/11)

- 追加
  - 入力補完で、ある程度状況に合うものを表示するようにしました
  - カーソルを合わせた位置の小節数がわかるようになりました（下部ステータスバーに表記）
  - 指定した小節に移動できるようになりました（下部ステータスバー小節数クリック）
  - 構文エラーが表示されるようになりました
- 修正
  - 入力補完の不具合を修正
  - ホバーの不具合を修正
  - その他細かい修正
- 仕様変更
  - 内部実装を変更（構文解析の実装）
    - より細やかな処理を実装できるようになりました
  - 譜面が不要にハイライトされるのを抑制しました
- 既知の不具合
  - 譜面分岐する場合、誤った小節数が表示されます
  - 同一難易度で一人プレイ用と二人プレイ用が分けられている場合、または段位譜面の場合、正常に構文解析されません

## 1.1.9 (2023/02/22)

- 仕様変更
  - 一部の命令補完でスペースまで出力するように変更

## 1.1.8 (2023/02/15)

- 仕様変更
  - 0の色調を変更しました（少し明るくなりました）
  - 拡張機能の更新履歴を別ファイルに分けました

## 1.1.7 (2023/02/15)

- 修正
  - 1.1.6公開時のミスを修正

## 1.1.6 (2023/02/15)

- 修正
  - 命令の補完が出にくかったのを修正

## 1.1.5 (2023/01/22)

- 修正
  - \#BRANCHSTARTのパラメータの説明が消えていたのを修正

## 1.1.4 (2023/01/22)

- 追加
  - SONG:ヘッダーを追加
- 仕様変更
  - インデントされた命令と譜面に対応
- 修正
  - EXAN:ヘッダーに暫定対応
  - ファイル内に出現するテキストが入力補完に表示されないように修正
  - ヘッダーの自動補完の挙動修正

## 1.1.3 (2023/01/21)

- 追加
  - 簡易的なパンくずリストの実装

## 1.1.2 (2023/01/21)

- 修正
  - ヘッダーの入力補完の有効化・無効化が動作していなかったのを修正

## 1.1.1 (2023/01/21)

- 修正
  - READMEの修正

## 1.1.0 (2023/01/21)

- 追加
  - ヒントを実装（ヘッダー+命令）
  - 入力補完時にもヒントが出るようにしました
  - \#を1文字打つだけで補完が出るようにしました
- 仕様変更
  - 入力補完の内部実装をTypeScriptに変更
  - ユーティリティ系の入力補完を一時的に削除
- 修正
  - ヘッダーと命令が行頭以外でも入力補完が効いていたのを修正
  - ヘッダー先頭の++--色付けをSUBTITLE:のみに修正
  - 説明のスペルミス修正
  - 一部表現を修正
  - VSCodeの全角スペースハイライト設定を無効化
  - 一部サンプルコードが動いていたのを削除
- 既知の不具合
  - EXAM:ヘッダーがうまく動きません

## 1.0.1 (2023/01/17)

- 追加
  - ヒントを実装（命令のみ）
- 仕様変更
  - 命令の色を青→紫に変更
    - 譜面部分と被らない色という意図です
  - ヘッダーと命令を大文字のみに制限
- 修正
  - 分岐関係の命令で色が変わっていなかったものを修正
  - 間違えて#EXAM命令が生まれていたので削除
  - 連打の後に#END命令が来ても連打の色が続いていたのを修正

## 1.0.0 (2023/01/15)

- 初期リリース